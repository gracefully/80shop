...
function act_user_info($pagesize=5,$field='title,code'){
	...
	elseif($do == 'tbnick'){
		if ($_POST['sub'] != '') {
			...
			$tbnick=implode(',',$_POST['tbnick']);
			
			if($dduser['tbnick']!=$tbnick){
				$duoduo->update(MOD,array('trade_uid'=>''),'id="'.$dduser['id'].'"');
				$duoduo->delete('trade_uid','uid="'.$dduser['id'].'"');
				
				$trade_uid_arr=get_4_tradeid($tbnick);
