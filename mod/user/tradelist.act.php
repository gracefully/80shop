...
function act_user_tradelist($pagesize = 10) {
	...
	if ($do == 'taobao') {
		...
		$dingdan = $duoduo->select_all('tradelist', '*', "uid=" . $dduser['id'] . " and (checked=2 or checked=3) and del=0 order by id desc limit $frmnum,$pagesize");
		foreach($dingdan as $k=>$row){
			$baobei_id=$duoduo->select('baobei','id as baobei_id','trade_id="'.$row['id'].'"');
			$dingdan[$k]['baobei_id']=$baobei_id;
		}
	}
	elseif ($do == 'lost') {
		if (isset ($_GET['q'])) {
			...
			$total = $duoduo->count('tradelist', 'uid=0 and del=0' . $where);
	...
	elseif ($do == 'gametask') {
		$total = $duoduo->count('gametask', 'memberid="' . $dduser['id'] . '" and del=0');
		$dingdan = $duoduo->select_all('gametask', '*', 'memberid="' . $dduser['id'] . '" and del=0 order by  id desc limit ' . $frmnum . ',' . $pagesize);
	}
	elseif ($do == 'task') {
