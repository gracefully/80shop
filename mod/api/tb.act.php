...
if($do=='go'){ //登陆
    $urls = 'https://oauth.taobao.com/authorize?response_type=code&client_id='.$app['key'].'&redirect_uri=http%3A%2F%2F'.URL.'%2Findex.php%3Fmod%3Dapi%26act%3Dtb%26do%3Dback&view=web';
...
elseif($do=='back'){  //回调
	...
	$row=$duoduo->select('apilogin','id,uid,webid','webname="'.$nick_taobao.'" and web="'.ACT.'"');
	if($row['id']>0){
