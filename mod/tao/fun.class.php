...
class dd_tao_class{
	...
	function dd_tao_goods($data){ //如果cid为a，检索推荐数据
    	...
    	$tao_goods=$this->duoduo->select_all('goods','*',$where.' order by sort asc,id desc limit '.$frmnum.','.$num);
	...
	function dd_tao_shops($shops){ //处理淘宝店铺数据
		...
		foreach ($shops as $k=>$row) {
			...
			$row['jump'] = u('jump','shop',array('sid'=>$row["sid"],'user_id'=>$row["user_id"],'nick'=>$row["nick"],'pic'=>$row["logo"]));
