...
function dd_json_encode($value){
	$jsonp=0;
	
	if(isset($_GET['callback'])){
		$callback=$_GET['callback'];
		if(preg_match('/^jQuery[0-9_]+/',$callback)){
			$jsonp=1;
		}
	}
	
	if(DDJSON==1 || !function_exists('json_encode')){
		$json = fs('DD_Services_JSON');
		if($jsonp==1){
			return $callback.'('.$json->encode($value).')';
		}
		else{
...
class DD_Services_JSON
{
                ...
                */
                ...
                if (is_array($var) && count($var) && (array_keys($var) !== range(0, sizeof($var) - 1))) {
                    ...
                    foreach($properties as $property) {
                        if(DD_Services_JSON::isError($property)) {
                            return $property;
                        }
                    }
                ...
                foreach($elements as $element) {
                    if(DD_Services_JSON::isError($element)) {
                        return $element;
                    }
                }
    ...
    */
    function name_value($name, $value)
    {
        ...
        if(DD_Services_JSON::isError($encoded_value)) {
