...
class zhidemai{
	...
	function do_item($item){
		...
		if(isset($item['shuxing'])){
			...
			$item['shuxing_id']=$item['shuxing_id'];
		}
	...
	function jump($id){
		$zhidemai=$this->duoduo->select($this->table_name,'web,url,uid','id="'.$id.'"');
		$web=$zhidemai['web'];
		$url=$zhidemai['url'];
		$uid=$zhidemai['uid'];
		if($web==1){
			$domain=get_domain($url);
			$mall_id=(int)$this->duoduo->select(get_mall_table_name(),'id','domain = "'.$domain.'"');
