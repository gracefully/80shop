...
<table cellspacing="0" width="100%" style="border:1px  solid #DCEAF7; border-bottom:0px; background:#E9F2FB">
        <tr>
              <td width="500" class="bigtext">&nbsp;<img src="images/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<?=u('mall_order','addedi')?>" class="link3">[添加订单]</a>&nbsp;<?php if($reycle==1){?><a href="<?=u(MOD,ACT)?>" class="link3">[站内列表]</a><?php }else{?><a href="<?=u(MOD,ACT,array('reycle'=>1))?>" class="link3">[回收站]</a><?php }?> <a href="<?=u(MOD,'get')?>" class="link3">[获取订单]</a></td>
              <td width=""></td>
              <td width="700" align="right" class="bigtext"><input type="text" name="q" value="<?=$q?>" />&nbsp;<?=select($malls,$mall_id,'mall_id')?>&nbsp;<?=select($select_arr,$se,'se')?>&nbsp;<?=select($status_arr,$status,'status')?>&nbsp;<?=select($select2_arr,$se2,'se2')?>&nbsp;<input type="submit" value="提交" /> 共有 <b><?php echo $total;?></b> 条记录&nbsp;&nbsp;</td>
            </tr>
      </table>
