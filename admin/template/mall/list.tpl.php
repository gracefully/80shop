...
$mall_type=0+$this_type;
?>
<form name="form1" action="" method="get">
<table cellspacing="0" width="100%" style="border:1px  solid #DCEAF7; border-bottom:0px; background:#E9F2FB">
        <tr>
              <td width="400px">&nbsp;<img src="images/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<?=u(MOD,'addedi')?>" class="link3">[新增商城]</a>&nbsp;<?php if($ddmall==0){?><a onclick='return confirm("确定要清空?")' href="<?=u(MOD,'list',array('del'=>1))?>" class="link3">[清空商城数据]</a> <a href="<?=u(MOD,'list',array('update'=>1))?>" class="link3">[获取商城数据]</a><?php }?> &nbsp;<?php if($reycle==1){?><a href="<?=u(MOD,ACT)?>" class="link3">[站内列表]</a><?php }else{?><a href="<?=u(MOD,ACT,array('reycle'=>1))?>" class="link3">[回收站]</a><?php }?></td>
      ...
      <form name="form2" method="get" action="" style="margin:0px; padding:0px">
            ...
            <input type="hidden" name="mod" value="<?=MOD?>" /><input type="hidden" name="act" value="del" /><input type="hidden" name="table" value="<?=$mall_class->get_table()?>" />
           <?php if($reycle==1){?>
