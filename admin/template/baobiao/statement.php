...
if(!defined('INDEX') && !defined('ADMIN')){
	...
	function plugin_statement_insert(){//list日期更新
		...
		//检索最早的一条订单时间
		$tbtime=$duoduo->select('tradelist','pay_time','checked="2" and pay_time<>"" order by pay_time asc');
		$tasktime=$duoduo->select('task','addtime','(immediate = "1" or immediate = "2") order by addtime asc');
		$gametime=$duoduo->select('gametask','addtime','1 order by addtime asc');
		$stime=$tbtime>$tasktime?$tasktime:$tbtime;
		$stime=$stime>$gametime?$gametime:$stime;
		if($stime!=''){
	...
	function plugin_statement_update($id_arr){//数据更新
		...
		foreach($id_arr as $id){
			...
			//游戏返利
			$where='addtime >= "'.$stime.'" and addtime < "'.$dtime.'"';
			$game=$duoduo->sum('gametask','money,point',$where);
			$data['gameyj']=$game['money'];
			$data['gamelr']=$data['gameyj']-$game['point'];
