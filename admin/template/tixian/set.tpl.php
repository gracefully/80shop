...
$(function(){
	...
	$('#getddshouquan').jumpBox({  
		...
		height:140,
		width:450,
		contain:'<div id="ddform">平台登陆密码：<input id="ddopenpwd" type="password" name="ddopenpwd" value="" /> <input type="submit" onclick="shouquan($(this))" value="登录获取" /></div>',
		LightBox:'show'
    });
})
...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    ...
    <td>&nbsp;<?=html_radio(array('0'=>'关闭',1=>'开启'),$webset['tixian']['ddpay'],'tixian[ddpay]')?> <span class="zhushi">后台确认集分宝提现，由统一接口为您代发 &nbsp;<a href="<?=DD_OPEN_JFB_REG_URL?><?=urlencode(URL)?>">开通权限</a></span></td>
  </tr>
  <tr id="ddkey" style="display:none">
    <td align="right">平台集分宝通信字符串：</td>
    <td>&nbsp;<input name="tixian[key]" id="tixian_key" value="<?=$webset['tixian']['key']?>" /> <span style="cursor:pointer; color:red; font-weight:bold; " id="getddshouquan">【获取数据】</span> <span class="zhushi"><a href="<?=DD_OPEN_JFB_REG_URL?><?=urlencode(URL)?>">查看</a>  作为平台与您的通信证书</span></td>
  </tr>
  <tr>
    ...
    <td>&nbsp;<label><input value="1" <?php if($webset['tixian']['need_alipay']==1){?> checked="checked"<?php }?> type="checkbox" name="tixian[need_alipay]" />支付宝</label> 
              ...
              <label><input value="1" <?php if($webset['tixian']['need_bank']==1){?> checked="checked"<?php }?> type="checkbox" name="tixian[need_bank]" />银行</label>&nbsp;&nbsp; <span class="zhushi"><a href="index.php?mod=tixian&amp;act=bank">银行设置</a> </span>   </td>
  </tr>
  <tr>
     <td align="right">&nbsp;</td>
