...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    ...
    <div style="line-height:28px"><label class="children">└─<input node="<?=$row['node']?>" class="<?=$row['node']?>" <?php if($role_menu_arr[$v['id']]){?> checked="checked"<?php }?> type="checkbox" name="ids[]" value="<?=$v['id']?>" />&nbsp;<span style="font-size:12px;font-family:wingdings">2</span>&nbsp;<?=$v['title']?><span class="zhushi">(模块：<?=$v['mod']?> 行为：<?=$v['act']?>)</span></label></div>
    <?php }?>
	<?php }?>
    </td>
  </tr>
