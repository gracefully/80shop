...
if(isset($_GET['sub']) || $_GET['page_no']!=''){
	...
	$url = 'index.php?mod=task&act=report';
	if($a['s']==0){
		jump(-1,$a['r']);
	}
	$info_arr=$a['r']['result'];
	...
	foreach($info_arr as $row){
			...
			$mingxi_id=$duoduo->select('mingxi','id','shijian="task_1" and source="'.$row['eventid'].'"');
			if(empty($info)){
				unset($data);
				...
				//给会员结算
				if(($row['immediate']==1 || $row['immediate']==2) && empty($mingxi_id)){
					unset($arr);
					$arr=array('f'=>'money','e'=>'+','v'=>$row['point']);
					$duoduo->update('user',$arr,'id="'.$row['mid'].'"');
					//插入明细
					unset($data);
					$data=array('uid'=>$row['mid'],'shijian'=>'task_1','money'=>$row['point'],'source'=>'任务名：'.$row['programname']);
