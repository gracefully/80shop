...
if ($_POST['sub'] != '' || $_GET['sub'] != '') {
	...
	if ($do == 4) { //多麦
		...
		$eday = date('Y-m-d', strtotime(trim($eday)) + 24 * 60 * 60); //多麦当天的不算的必须加一天

		$url = 'http://www.duomai.com/api/order.php?format=xml&action=query&hash=' . $webset['duomai']['key'] . '&site_id=' . $webset['duomai']['wzid'] . '&time_from=' . $sday . '&time_to=' . $eday . '&limit=0,10000,order=time,desc';
		$arr = dd_get_xml($url);
	    ...
	    for($i=$fromnum;$i<$maxnum;$i++){
			...
			$mall=$duoduo->select('mall','id,title,type','duomaiid="'.$ads_id.'"');
			$ads_name = $mall['title']; //活动名
			$site_id = $row['site_id']; //网站ID
			$link_id = $row['link_id']; //活动链接ID
			$uid = (int) $row['euid']; //	网站主设定的反馈标签
	...
	if ($do == 2) { //领克特
		...
		for($i=$fromnum;$i<$maxnum;$i++){
			...
			$mall=$duoduo->select('mall','id,title,type','merchant="'.$merchant_id.'"');
			$mall_name = $mall['title']; //活动名
			$uid = (int) $row['u_id']; //	网站主设定的反馈标签
	...
	if ($do == 3) { //亿起发
	    ...
	    for($i=$fromnum;$i<$maxnum;$i++){
			...
			if ($row != '' && !is_array($row)) {
				...
				$unique_id = $arr[0];
				$adid = $arr[1];
				$mall=$duoduo->select('mall','id,title,type','yiqifaid="'.$adid.'"');
				$mall_name=$mall['title'];
	...
	if ($do == 1) { //成果
		...
		if($c>0){
			...
			for($i=$fromnum;$i<$maxnum;$i++){
				...
				$mall=$duoduo->select('mall','id,title,type','chanetid="'.$adid.'"'); //活动名
			    $mall_name = $mall['title'];
				
				if(is_numeric($row['user_info'])){
					$uid = (int)$row['user_info']; //	网站主设定的反馈标签
				}
				else{
				    $uid=0;
				}
