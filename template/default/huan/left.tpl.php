<div id="apDiv9">
    ...
    <?php }else{?>
      <div id="apDiv22"></div>
      <div id="apDiv23"><a onclick="window.location='<?=u('user','login')?>&url='+encodeURIComponent(window.location.href)" style="cursor:pointer"><img alt="登陆" src="<?=TPLURL?>/images/fqw_dl.gif" width="171" height="35" /></a></div>
    <?php }?>
    </div>
...
<ul>
  ...
  <li><a <?php if($cid==$catid){?> class="current"<?php }?> href="<?=u('huan','list',array('cid'=>$catid))?>"><?=$title?></a></li>
  <?php }?>
</ul>
</div>
</div>