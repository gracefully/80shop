...
<?=AD(10)?>
  ...
  <td width="57%" valign="top">
    ...
    <?=TBMONEY?>兑换 ：<b style=" color:#FF6600; font-size:16px"><?=jfb_data_type($good['jifenbao'])?></b>&nbsp;
    <?php }else{?>
    <?=TBMONEY?>兑换 ：不参与
    <?php }?>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<?php if($dduser['name']!=''){?>
	<?php if($good['jifenbao']>0){?>
	<button class="money"  id="jifenbao" title="<?=$jifenbao_dh_msg?>" <?php if($jifenbao_dh_status==0){?> disabled="disabled"<?php }?>><?=TBMONEY?>兑换</button>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<?php }?>
	<?php if($good['jifen']>0){?>
...
<?php }else{?>
	<?php if($good['jifenbao']>0){?>
	<button class="money" onclick="alert('登陆后才可兑换商品！');window.location='<?=u('user','login')?>&url='+encodeURIComponent(window.location.href)"><?=TBMONEY?>兑换</button>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<?php }?>
	<?php if($good['jifen']>0){?>
	<button class="jifen" onclick="alert('登陆后才可兑换商品！');window.location='<?=u('user','login')?>&url='+encodeURIComponent(window.location.href)">积分兑换</button>
	<?php }?>
<?php }?>
</div></td></tr></table></div>
...
<script>
$(function(){
	<?php if($jifenbao_dh_status==0){?>
	$('#jifenbao').attr('disable','true');
	<?php }?>
...
<form id="form1" name="form1" method="post" action="<?=u('ajax','huan')?>">
                        ...
                        <tr>
                            <td width="70" height="35" align="right">兑换数量：</td>
