    ...
    <?php include(TPLPATH."/user/top.tpl.php");?>
      ...
      <input type="hidden" name="mod" value="user" /><input type="hidden" name="act" value="msg" /><input type="hidden" name="do" value="del" />
                    <table width="770" border="0" cellpadding="0" cellspacing="1">
                      ...
                      <tr>
                        <td height="33" align="center"><img src="<?=TPLURL?>/images/msg<?=$r['see']?>.gif" width="14" height="10" /></td>
                        <td><input class="checkbox" type='checkbox' name='ids[]' value='<?=$r["id"]?>' id='content_<?=$r["id"]?>' /></td>
                        <td><a class="read" id="<?=$r["id"]?>" href="<?=u('ajax','get_msg',array('id'=>$r["id"]))?>"><?=utf_substr($r["content"],64)?>......</a></td>
                        <td><?=$r["addtime"]?></td>
              ...
              <?php }?>
                ...
                <FORM onsubmit="if($('#content').val()==''){alert('内容不能为空');return false;}else{return true;}" method=post action="index.php?mod=user&act=msg&do=save_send">
