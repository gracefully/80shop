...
if($dduser['txstatus']=='1'){
	$txstate_msg = "您当前有一笔提现申请正在处理当中，请注意查收！ <a href='".u('user','mingxi',array('do'=>'out'))."'>>>查看详细</a>";
}else{
	if($dduser['money']==0){
		$txstate_msg = "感谢您使用".WEBNAME."，当您购物累积获得返现超过".$webset['tixian']['limit']."元，就可以申请提现。祝您购物愉快！";
	}elseif($dduser['live_money']<$webset['tixian']['limit']){
		$txstate_msg = '亲！您当前的可用余额是 <span>'.$dduser['live_money'].'</span> 元，还差 <span>'.($webset['tixian']['limit']-$dduser['live_money']).'</span> 元就可以申请提现了！';
	}else{
		$txstate_msg = "亲！您当前的可用余额是<span>".$dduser['live_money']."</span> 元，可以申请提现了！&nbsp;&nbsp;&nbsp;&nbsp;<img style='margin-bottom:-10px' src='images/face/2.gif'/><a href='".u('user','tixian',array('type'=>2))."'><b style='color:red;cursor:pointer'>申请提现</b>>>></a>";
	}
}
...
if(JIFENBAO==2){
	...
	if($dduser['tbtxstatus']=='1'){
		$tbtxstate_msg = "您当前有一笔".TBMONEY."提现申请正在处理当中，我们将支付到您的支付宝中，请注意查收！ <a href='".u('user','mingxi',array('do'=>'out'))."'>>>查看详细</a>";
	}else{
		if($dduser['jifenbao']==0){
			$tbtxstate_msg = "感谢您使用".WEBNAME."，当您购物累积获得返现超过".$webset['tixian']['tblimit'].TBMONEYUNIT.TBMONEY."，就可以申请提现。祝您购物愉快！";
		}elseif($dduser['live_jifenbao']<$webset['tixian']['tblimit']){
			$tbtxstate_msg = '亲！您当前的可用'.TBMONEY.'是 <span>'.$dduser['live_jifenbao'].TBMONEYUNIT.'</span>，还差 <span>'.($webset['tixian']['tblimit']-$dduser['live_jifenbao']).'</span>'.TBMONEY.'就可以申请提现了！';
		}else{
			$tbtxstate_msg = "亲！您当前的可用".TBMONEY."是<span>".$dduser['live_jifenbao'].TBMONEYUNIT."</span>，可以申请提现了！&nbsp;&nbsp;&nbsp;&nbsp;<img style='margin-bottom:-10px' src='images/face/2.gif'/><a href='".u('user','tixian',array('type'=>1))."'><b style='color:red;cursor:pointer'>申请提现</b>>>></a>";
		}
	}
}
                ...
                <div class="adminright_user">
                    ...
                    <div class="adminright_user_main">
                      ...
                      <div class="adminright_user_main_r">
                          <p>可用余额：<span><?=$dduser['live_money']?></span> 元 (未结算：<?=$dduser['freeze_money']?> 元) <a href="<?=u('user','mingxi')?>"> 收入明细>></a> <?php if($webset['taoapi']['m2j']==1){?><a id="huanqian" style="cursor:pointer">转换成<?=TBMONEY?></a><?php }?></p>
                          <p>可用<?=TBMONEY?>：<span><?=$dduser['live_jifenbao']?></span> <?=TBMONEYUNIT?>(未结算：<?=$dduser['freeze_jifenbao']?> <?=TBMONEYUNIT?>) <a href="<?=u('user','mingxi')?>"> 收入明细>></a> <?php if(JIFENBAO==1){?>&nbsp;<a target="_blank" href="https://jf.alipay.com/prod/integral.htm">查看集分宝余额</a><?php }?> </p>
                          <p>可用积分：<span><?=$dduser['live_jifen']?></span> 点 (未结算：<?=$dduser['freeze_jifen']?> 点)    <a href="<?=u('user','huan')?>">兑换明细>></a></p>
                          <p>已提金额：<span><?=$dduser['yitixian']?> 元</span>&nbsp;&nbsp;&nbsp;&nbsp;已提<?=TBMONEY?>：<span><?=jfb_data_type($dduser['tbyitixian'])?></span> <?=TBMONEYUNIT?>    &nbsp;已兑换<?=TBMONEY?>：<span><?=(float)$spend_jifenbao?> <?=TBMONEYUNIT?></span>&nbsp;&nbsp;&nbsp;&nbsp;已兑换积分：<span><?=$spend_jifen?></span> 个</p>
...
$(function(){
    $('#huanqian').jumpBox({  
	    title: '温馨提示：按照1比<?=TBMONEYBL?>的比例将现金转换成<?=TBMONEY?>！<?php if(JFB_FEE>0){?><?=TBMONEY?>兑换需要额外支付<b style=" color:red"><?=JFB_FEE*100?>%</b>的手续费<?php }?>',
		titlebg:1,
		height:200,
		width:580,
		defaultContain:1
