...
$bank_tr='<tr class="tool_tr"><td width="" height="35" align="right">提现银行：</td><td align="left">'.str_replace("'","\'",select($bank_arr,$bank_id,'bank_id',$bank_code_dis,1)).'</td><td width="" align="left"><label class="field_notice"></label></td></tr><tr class="tool_tr"><td width="" height="35" align="right">银行账号：</td><td align="left"><input name="bank_code" '.$bank_code_dis.' type="text" class="ddinput" value="'.$dduser['bank_code'].'" id="bank_code" style="width:160px;" /></td><td width="" align="left"><label class="field_notice">您的银行账号</label></td></tr>';
...
$(function(){	
	...
	$('#form1').validate({
        ...
        messages : {
            money : {
                required : '提现<?=$money_name?>必填',
				range    : '提现<?=$money_name?>应大于<?=$tixian_limit?><?php if($tixian_limit<$max_money){echo '且小于'.$max_money;} ?>',
                dd_multiple : '提现<?=$money_name?>不是<?=$txxz?>的整数倍'
            },
			mobile : {
                mobile : '手机号码格式错误'
            },
		...
		submitHandler: function(form) {
			...
			//ajaxPostForm(form,'<?=u('user','index')?>','提现成功，等待审核');
...
<div class="mainbody">
    ...
    <?php include(TPLPATH."/user/top.tpl.php");?>
    	<div class="adminmain">
            ...
            <form id="form1" name="form1" autocomplete="off" method="post" action="<?=l('user','tixian',array('type'=>$type))?>">
            <input type="hidden" name="sub" value="1" />
                        <table width="605" border="0" cellpadding="0" cellspacing="0">
                          ...
                          <tr>
                            ...
                            <td width="" align="left"><label class="field_notice"><?php if(count($tx_tr_arr)>1){?><a href="<?=u('user','info',array('do'=>'caiwu'))?>" style="text-decoration:underline">修改默认提现平台</a><?php }?></label></td>
                          </tr>
						  ...
						  <tr>
                            ...
                            <td width="" align="left"><label class="field_notice"><?php if($webset['sms']['open']==1){?>填写后，提现成功，会免费短信通知您<?php }else{?>填写您的手机号码<?php }?></label></td>
                          </tr>
						  <tr>
