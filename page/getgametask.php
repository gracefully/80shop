...
if(isset($_GET['eventid']) && $_GET['eventid']!=''){
	...
	if($eventid!='' && !empty($key) && $passcode==$pass){
		//入数据库
		$info=$duoduo->select('gametask','id','eventid="'.$eventid.'"');
			unset($data);
		...
		if(empty($info)){
			...
			//给会员结算
				unset($arr);
				$arr=array('f'=>'money','e'=>'+','v'=>$point);
				$duoduo->update('user',$arr,'id="'.$memberid.'"');
				//插入明细
				$data=array('uid'=>$memberid,'shijian'=>'gametask_1','money'=>$point,'source'=>'任务名：'.$programname);
				$duoduo->mingxi_insert($data);
		}else{
			$duoduo->update('gametask',$data,'id="'.$info.'"');
			$re=array('s'=>4,'r'=>'更新成功');
		}
	}else{
		$re=array('s'=>3,'r'=>'验证失败');
