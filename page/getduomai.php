...
$dir =DDROOT.'/data/duomai_'.substr(md5(DDKEY),0,16).'/'. date("Y").'/'.date('md').'.txt';
...
$checksum_md5=md5($_GET['ads_id'].$_GET['ads_name'].$_GET['euid'].$_GET['link_id'].$_GET['order_sn'].$_GET['order_time'].$_GET['orders_price'].$_GET['site_id'].$_GET['siter_commission'].$_GET['status'].$webset['duomai']['key']);

if($checksum_md5!=$_GET['checksum']){
    exit('hack');
}

if($site_id!=$webset['duomai']['wzid']){
    exit('非本站订单！');
}
